...
export const Concept = () => {
    ...
    const solvedVal = (state1?2:0)+(state0?1:0)
    ...
    <span></span>
    <span>
        code bit 1 = <input value={stateToVal(state1)} onChange={e=>setState1(valToState(e.currentTarget.value.slice(-1)))} style={inputCss}/><br/>
        ({state1?"positive":"negative"})
    </span>
    <span></span>
    <span>
        code bit 0 = <input value={stateToVal(state0)} onChange={e=>setState0(valToState(e.currentTarget.value.slice(-1)))} style={inputCss}/><br/>
        ({state0?"positive":"negative"})
    </span>
    <span></span>
    <span>code = B{state1?"1":"0"}{state0?"1":"0"} = <input 
        value={solvedVal} 
        onChange={e=>{
            const val = parseInt(e.currentTarget.value.slice(-1))
            setState1(val===2 || val===3)
            setState0(val===1 || val===3)
        }}
        style={inputCss}
    /></span>
    ...
    <span> X </span>
    <img src={state1?pos1:neg1} alt="stacked1"/>
    <span> X </span>
    <img src={state0?pos0:neg0} alt="stacked0"/>
    <span> = </span>
