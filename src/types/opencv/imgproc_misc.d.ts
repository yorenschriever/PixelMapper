 ...
 */
  function adaptiveThreshold(src: InputArray, dst: OutputArray, maxValue: double, adaptiveMethod: int, thresholdType: int, blockSize: int, C: double): void
 ...
 */
  function blendLinear(src1: InputArray, src2: InputArray, weights1: InputArray, weights2: InputArray, dst: OutputArray): void
 ...
 */
  function distanceTransform(src: InputArray, dst: OutputArray, labels: OutputArray, distanceType: int, maskSize: int, labelType?: int): void
 ...
 */
  function distanceTransform(src: InputArray, dst: OutputArray, distanceType: int, maskSize: int, dstType?: int): void
 ...
 */
  function floodFill(image: InputOutputArray, seedPoint: Point, newVal: Scalar, rect?: any, loDiff?: Scalar, upDiff?: Scalar, flags?: int): int
 ...
 * FloodFillFlags.
 */
  function floodFill(image: InputOutputArray, mask: InputOutputArray, seedPoint: Point, newVal: Scalar, rect?: any, loDiff?: Scalar, upDiff?: Scalar, flags?: int): int
 ...
 */
  function grabCut(img: InputArray, mask: InputOutputArray, rect: Rect, bgdModel: InputOutputArray, fgdModel: InputOutputArray, iterCount: int, mode?: int): void

/**
 * This is an overloaded member function, provided for convenience. It differs from the above function
 * only in what argument(s) it accepts.
 */
  function integral(src: InputArray, sum: OutputArray, sdepth?: int): void

/**
 * This is an overloaded member function, provided for convenience. It differs from the above function
 * only in what argument(s) it accepts.
 */
  function integral(src: InputArray, sum: OutputArray, sqsum: OutputArray, sdepth?: int, sqdepth?: int): void
 ...
 */
  function integral(src: InputArray, sum: OutputArray, sqsum: OutputArray, tilted: OutputArray, sdepth?: int, sqdepth?: int): void
 ...
 */
  function threshold(src: InputArray, dst: OutputArray, thresh: double, maxval: double, type: int): double
 ...
 * size as image .
 */
  function watershed(image: InputArray, markers: InputOutputArray): void
 ...
 * 
 */
  const ADAPTIVE_THRESH_MEAN_C: AdaptiveThresholdTypes // initializer: = 0
 ...
 * 
 */
  const ADAPTIVE_THRESH_GAUSSIAN_C: AdaptiveThresholdTypes // initializer: = 1
 ...
 * 
 */
  const DIST_LABEL_CCOMP: DistanceTransformLabelTypes // initializer: = 0

/**
 * each zero pixel (and all the non-zero pixels closest to it) gets its own label.
 * 
 */
  const DIST_LABEL_PIXEL: DistanceTransformLabelTypes // initializer: = 1

  const DIST_MASK_3: DistanceTransformMasks // initializer: = 3

  const DIST_MASK_5: DistanceTransformMasks // initializer: = 5

  const DIST_MASK_PRECISE: DistanceTransformMasks // initializer: = 0

  const DIST_USER: DistanceTypes // initializer: = -1

  const DIST_L1: DistanceTypes // initializer: = 1

  const DIST_L2: DistanceTypes // initializer: = 2

  const DIST_C: DistanceTypes // initializer: = 3

  const DIST_L12: DistanceTypes // initializer: = 4

  const DIST_FAIR: DistanceTypes // initializer: = 5

  const DIST_WELSCH: DistanceTypes // initializer: = 6

  const DIST_HUBER: DistanceTypes // initializer: = 7
 ...
 * 
 */
  const FLOODFILL_FIXED_RANGE: FloodFillFlags // initializer: = 1 << 16
 ...
 * 
 */
  const FLOODFILL_MASK_ONLY: FloodFillFlags // initializer: = 1 << 17

  const GC_BGD: GrabCutClasses // initializer: = 0

  const GC_FGD: GrabCutClasses // initializer: = 1

  const GC_PR_BGD: GrabCutClasses // initializer: = 2

  const GC_PR_FGD: GrabCutClasses // initializer: = 3
 ...
 * 
 */
  const GC_INIT_WITH_RECT: GrabCutModes // initializer: = 0
 ...
 * 
 */
  const GC_INIT_WITH_MASK: GrabCutModes // initializer: = 1

/**
 * The value means that the algorithm should just resume.
 * 
 */
  const GC_EVAL: GrabCutModes // initializer: = 2
 ...
 * 
 */
  const GC_EVAL_FREEZE_MODEL: GrabCutModes // initializer: = 3

  const THRESH_BINARY: ThresholdTypes // initializer: = 0

  const THRESH_BINARY_INV: ThresholdTypes // initializer: = 1

  const THRESH_TRUNC: ThresholdTypes // initializer: = 2

  const THRESH_TOZERO: ThresholdTypes // initializer: = 3

  const THRESH_TOZERO_INV: ThresholdTypes // initializer: = 4

  const THRESH_MASK: ThresholdTypes // initializer: = 7

  const THRESH_OTSU: ThresholdTypes // initializer: = 8

  const THRESH_TRIANGLE: ThresholdTypes // initializer: = 16
