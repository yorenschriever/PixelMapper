 ...
 */
  function approxPolyDP(curve: InputArray, approxCurve: OutputArray, epsilon: double, closed: bool): void
 ...
 */
  function arcLength(curve: InputArray, closed: bool): double
 ...
 */
  function boundingRect(array: InputArray): Rect
 ...
 */
  function boxPoints(box: RotatedRect, points: OutputArray): void
 ...
 */
  function connectedComponents(image: InputArray, labels: OutputArray, connectivity: int, ltype: int, ccltype: int): int
 ...
 * @param ltype output image label type. Currently CV_32S and CV_16U are supported.
 */
  function connectedComponents(image: InputArray, labels: OutputArray, connectivity?: int, ltype?: int): int
 ...
 */
  function connectedComponentsWithStats(image: InputArray, labels: OutputArray, stats: OutputArray, centroids: OutputArray, connectivity: int, ltype: int, ccltype: int): int
 ...
 * @param ltype output image label type. Currently CV_32S and CV_16U are supported.
 */
  function connectedComponentsWithStats(image: InputArray, labels: OutputArray, stats: OutputArray, centroids: OutputArray, connectivity?: int, ltype?: int): int
 ...
 */
  function contourArea(contour: InputArray, oriented?: bool): double
 ...
 */
  function convexHull(points: InputArray, hull: OutputArray, clockwise?: bool, returnPoints?: bool): void
 ...
 */
  function convexityDefects(contour: InputArray, convexhull: InputArray, convexityDefects: OutputArray): void

  function createGeneralizedHoughBallard(): any

  function createGeneralizedHoughGuil(): any
 ...
 * context.
 */
  function findContours(image: InputArray, contours: OutputArrayOfArrays, hierarchy: OutputArray, mode: int, method: int, offset?: Point): void

/**
 * This is an overloaded member function, provided for convenience. It differs from the above function
 * only in what argument(s) it accepts.
 */
  function findContours(image: InputArray, contours: OutputArrayOfArrays, mode: int, method: int, offset?: Point): void
 ...
 * @param points Input 2D point set, stored in std::vector<> or Mat
 */
  function fitEllipse(points: InputArray): RotatedRect
 ...
 * @param points Input 2D point set, stored in std::vector<> or Mat
 */
  function fitEllipseAMS(points: InputArray): RotatedRect
 ...
 * @param points Input 2D point set, stored in std::vector<> or Mat
 */
  function fitEllipseDirect(points: InputArray): RotatedRect
 ...
 */
  function fitLine(points: InputArray, line: OutputArray, distType: int, param: double, reps: double, aeps: double): void
 ...
 */
  function HuMoments(moments: any, hu: double): void

/**
 * This is an overloaded member function, provided for convenience. It differs from the above function
 * only in what argument(s) it accepts.
 */
  function HuMoments(m: any, hu: OutputArray): void

  function intersectConvexConvex(_p1: InputArray, _p2: InputArray, _p12: OutputArray, handleNested?: bool): float
 ...
 */
  function isContourConvex(contour: InputArray): bool
 ...
 */
  function matchShapes(contour1: InputArray, contour2: InputArray, method: int, parameter: double): double
 ...
 */
  function minAreaRect(points: InputArray): RotatedRect
 ...
 */
  function minEnclosingCircle(points: InputArray, center: any, radius: any): void
 ...
 */
  function minEnclosingTriangle(points: InputArray, triangle: OutputArray): double
 ...
 */
  function moments(array: InputArray, binaryImage?: bool): Moments
 ...
 */
  function pointPolygonTest(contour: InputArray, pt: Point2f, measureDist: bool): double
 ...
 */
  function rotatedRectangleIntersection(rect1: any, rect2: any, intersectingRegion: OutputArray): int

  const CCL_WU: ConnectedComponentsAlgorithmsTypes // initializer: = 0

  const CCL_DEFAULT: ConnectedComponentsAlgorithmsTypes // initializer: = -1

  const CCL_GRANA: ConnectedComponentsAlgorithmsTypes // initializer: = 1
 ...
 * 
 */
  const CC_STAT_LEFT: ConnectedComponentsTypes // initializer: = 0
 ...
 * 
 */
  const CC_STAT_TOP: ConnectedComponentsTypes // initializer: = 1

  const CC_STAT_WIDTH: ConnectedComponentsTypes // initializer: = 2

  const CC_STAT_HEIGHT: ConnectedComponentsTypes // initializer: = 3

  const CC_STAT_AREA: ConnectedComponentsTypes // initializer: = 4

  const CC_STAT_MAX: ConnectedComponentsTypes // initializer: = 5
 ...
 * 
 */
  const CHAIN_APPROX_NONE: ContourApproximationModes // initializer: = 1
 ...
 * 
 */
  const CHAIN_APPROX_SIMPLE: ContourApproximationModes // initializer: = 2

/**
 * applies one of the flavors of the Teh-Chin chain approximation algorithm TehChin89
 * 
 */
  const CHAIN_APPROX_TC89_L1: ContourApproximationModes // initializer: = 3

/**
 * applies one of the flavors of the Teh-Chin chain approximation algorithm TehChin89
 * 
 */
  const CHAIN_APPROX_TC89_KCOS: ContourApproximationModes // initializer: = 4

  const INTERSECT_NONE: RectanglesIntersectTypes // initializer: = 0

  const INTERSECT_PARTIAL: RectanglesIntersectTypes // initializer: = 1

  const INTERSECT_FULL: RectanglesIntersectTypes // initializer: = 2
 ...
 * 
 */
  const RETR_EXTERNAL: RetrievalModes // initializer: = 0

/**
 * retrieves all of the contours without establishing any hierarchical relationships.
 * 
 */
  const RETR_LIST: RetrievalModes // initializer: = 1
 ...
 * 
 */
  const RETR_CCOMP: RetrievalModes // initializer: = 2

/**
 * retrieves all of the contours and reconstructs a full hierarchy of nested contours.
 * 
 */
  const RETR_TREE: RetrievalModes // initializer: = 3

  const RETR_FLOODFILL: RetrievalModes // initializer: = 4

  const CONTOURS_MATCH_I1: ShapeMatchModes // initializer: =1

  const CONTOURS_MATCH_I2: ShapeMatchModes // initializer: =2

  const CONTOURS_MATCH_I3: ShapeMatchModes // initializer: =3
